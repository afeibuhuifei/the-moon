...
export function useSun({ segments = 64, radiusMultiplier = 0.3 }: UseSunProps = {}) {
	...
	const scaledRadius = sunRadius * radiusMultiplier * 0.001 // 缩小太阳以适应场景
	...
	const sunGlow = useMemo(() => {
		...
		glowSprite.scale.set(scaledRadius * 4, scaledRadius * 4, 1)
	...
	const sunLight = useMemo(() => {
		const light = new PointLight(0xffffff, 2.0, 1000)
